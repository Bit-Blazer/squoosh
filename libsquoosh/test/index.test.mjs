...
describe('squoosh-node', () => {
  ...
  for (const codec of codecs) {
    it(`should successfully encode an image using ${codec}`, async () => {
      ...
      assert.ok(result.binary instanceof Uint8Array, `${codec} output should be Uint8Array`);
      assert.ok(result.size > 0, `${codec} encoded size should be greater than 0`);
      assert.ok(result.binary.length > 0, `${codec} encoded binary should have data`);
      
      console.log(`\n  squoosh-node results (${codec}):`);
      console.log(`   Original: ${(fileBuffer.length / 1024).toFixed(2)} KB`);
      console.log(`   └ Output: ${(result.size / 1024).toFixed(2)} KB (↓${((1 - result.size / fileBuffer.length) * 100).toFixed(2)}%)`);
    });
  }
});
